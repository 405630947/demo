...
public class MonthActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fh = fh / 100;
        fh = fh * fh;
        TextView week1 = (TextView) findViewById(R.id.textView25);
        TextView week2 = (TextView) findViewById(R.id.textView26);
        TextView week3 = (TextView) findViewById(R.id.textView27);
        TextView week4 = (TextView) findViewById(R.id.textView28);
        ...
        if (cursor.getCount() < 35) {
            if (cursor.getCount() >= 7) {
                ...
                float fw = (float) changeweight1;    // 取得 體重輸入值
                NumberFormat nf = NumberFormat.getInstance();   // 數字格式
                nf.setMaximumFractionDigits(2);                 // 限制小數第一位
                fresult = fw / fh;
                String week1text = "原本體重:" + w + "公斤" + "\n" + "最終體重:" + changeweight1 + "公斤" + "\n" + "本週卡路里變化量:"+ "\n" + (0-(int)doubletotal1) + "大卡" + "\n" + "最終BMI:" + nf.format(fresult);
                week1.setText(week1text);
            }
            ...
            if (cursor.getCount() >= 14) {
                ...
                float fw2 = (float) changeweight2;    // 取得 體重輸入值
                NumberFormat nf = NumberFormat.getInstance();   // 數字格式
                nf.setMaximumFractionDigits(2);                 // 限制小數第一位
                fresult2 = fw2 / fh;
                String week2text = "上週體重:" + changeweight1 + "公斤" + "\n" + "最終體重:" + changeweight2 + "公斤" + "\n" + "本週卡路里變化量:"+ "\n" + (0-(int)doubletotal2)+ "大卡" + "\n" + "最終BMI:" + nf.format(fresult2);
                week2.setText(week2text);
            }
            if (cursor.getCount() >= 21) {
                ...
                float fw3 = (float) changeweight3;    // 取得 體重輸入值
                NumberFormat nf = NumberFormat.getInstance();   // 數字格式
                nf.setMaximumFractionDigits(2);                 // 限制小數第一位
                fresult3 = fw3 / fh;
                String week3text = "上週體重:" + changeweight2 + "公斤" + "\n" + "最終體重:" + changeweight3 + "公斤" + "\n" + "本週卡路里變化量:" + "\n"+ (0-(int)doubletotal3) + "大卡" + "\n" + "最終BMI:" + nf.format(fresult3);
                week3.setText(week3text);
            }
            if (cursor.getCount() >= 28) {
                for (int i = 21; i <= 27; i++) {
                    doubletotal4 += Double.parseDouble(arraytotal[i]);
                }
                changeweight4 = Double.parseDouble(df.format((Double.parseDouble(w) - ((doubletotal1 + doubletotal2 + doubletotal3 + doubletotal4) / 7700))));
                float fw4 = (float) changeweight4;    // 取得 體重輸入值
                NumberFormat nf = NumberFormat.getInstance();   // 數字格式
                nf.setMaximumFractionDigits(2);                 // 限制小數第一位
                fresult4 = fw4 / fh;
                String week4text = "上週體重:" + changeweight3 + "公斤" + "\n" + "最終體重:" + changeweight4 + "公斤" + "\n" + "本週卡路里變化量:"+ "\n" + (0-(int)doubletotal4) + "大卡" + "最終BMI:" + nf.format(fresult4);
                week4.setText(week4text);
            }
        }
        if(cursor.getCount()>=35) {
            int cnt = cursor.getCount();
            int latest = cnt - cnt % 7;
            int oldest = latest - 28;
            double[] arr = new double[4];
            for (int i = oldest; i < latest; i++) {
                int index = (i - oldest) / 7;
            ...
            changeweight4 = Double.parseDouble(df.format((Double.parseDouble(w) - ((doubletotal0 + arr[0] + arr[1] + arr[2] + arr[3]) / 7700))));
            float fw = (float) changeweight1, fw2 = (float) changeweight2, fw3 = (float) changeweight3, fw4 = (float) changeweight4;
            ...
            fresult3 = fw3 / fh;
            fresult4 = fw4 / fh;
            String week1text = "上週體重:" + changeweight0 + "公斤" + "\n" + "最終體重:" + changeweight1 + "公斤" + "\n" + "本週卡路里變化量:"+ "\n" + (0-(int)arr[0]) + "大卡" + "\n" + "最終BMI:" + nf.format(fresult);
            String week2text = "上週體重:" + changeweight1 + "公斤" + "\n" + "最終體重:" + changeweight2 + "公斤" + "\n" + "本週卡路里變化量:"+ "\n" + (0-(int)arr[1]) + "大卡" + "\n" + "最終BMI:" + nf.format(fresult2);
            String week3text = "上週體重:" + changeweight2 + "公斤" + "\n" + "最終體重:" + changeweight3 + "公斤" + "\n" + "本週卡路里變化量:"+ "\n" + (0-(int)arr[2]) + "大卡" + "\n" + "最終BMI:" + nf.format(fresult3);
        ...
        if(cursor.getCount()>=14) {
            ImageView iv2 = (ImageView) findViewById(R.id.imageView9);
            if ("男".equals(s)) {
                if (fresult2 <= 15) {
        ...
        if(cursor.getCount()>=21) {
            ImageView iv3 = (ImageView) findViewById(R.id.imageView10);
            if ("男".equals(s)) {
                if (fresult3 <= 15) {
        ...
        if(cursor.getCount()>=28) {
            ImageView iv4 = (ImageView) findViewById(R.id.imageView11);
            if ("男".equals(s)) {
                if (fresult4 <= 15) {
